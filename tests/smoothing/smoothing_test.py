...
from utils import obs_times_to_store, kalman_results_to_df, benchmark_pf_results_to_df, multismooth_results_to_df
...
FFBS_results = multiplexer(f=smoothing_worker, nruns=1, nprocs=0, seeding=True, method=['FFBS_MCMC'], N=N_FFBS_MCMC, fk=fks, smc_cls=SMC, quantiles=quantiles)
# results = genealogy_results + FFBS_results
results = FFBS_results
...
if part_3:
    ...
    FFBS_results = multiplexer(f=smoothing_worker, nruns=1, nprocs=0, seeding=True, method=['FFBS_MCMC'], N=N_FFBS_MCMC, fk=smth_fks, num=num, smc_cls=CDSSM_SMC, quantiles=quantiles)
    # results = genealogy_results + FFBS_results
    results = FFBS_results
...
results_df_1.to_json(f'./results/smoothing_test_run_{run_id}_{cdssm_str}_part_1.json', index=False)
results_df_2.to_json(f'./results/smoothing_test_run_{run_id}_{cdssm_str}_part_2.json', index=False)
if part_3:
    results_df_3.to_json(f'./results/smoothing_test_run_{run_id}_{cdssm_str}_part_3.json', index=False)
