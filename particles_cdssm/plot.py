"""
We use this module for some plotting utilities for the CD-SSM package.

The most useful functions will be:

`cdssm_simulation_plot` and `mv_cdssm_simulation_plot` for plotting the simulation of a CDSSM or MvCDSSM object, respectively.

We also have utilities to evaluate the performance of the multiple run particle filter/smoother methods:
...
def cdssm_simulation_plot(cdssm, x, y):
    """
    Plot the simulation of a (1D) NormalCDSSM object.
    """
...
def mv_cdssm_simulation_plot(cdssm, x, y):
    """
    Plot a simulation generated from a MvNormalCDSSM object.
