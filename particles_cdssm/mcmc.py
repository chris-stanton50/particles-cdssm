...
class MCMC(mcmc.MCMC):
    
    def store_x_n(self, x, n):
        """
        """
        self.chain.x[n] = x

    def prev_x_copy(self, n):
        """
        """
        return self.chain.x[n-1].copy()
...
class X_MCMC(MCMC):
    ...
    def to_idata(self, name=None):
        """
        Converts the MCMC chain output into an InferenceData object: used to process the MCMC output.
        """
        return to_idata(self, name=None)
...
class CDSSM_MCMC(MCMC):
    ...
    def build_chain_container_kwargs(self):
        cdssm = self.fk.cdssm
        data = self.fk.data
        if not cdssm.isequidistant:
            raise NotImplementedError("CDSSM-MCMC algorithms only implemented for CDSSMs with equidistant observation times.")
        if cdssm.isobservedat0:
            init_x = cdssm._init_dist_container(self.niter)
            x = cdssm.state_container(self.niter, len(data) - 1, self.num, cdssm.s_ts)
            chain_kwargs = {'x': x, 'init_x': init_x}
        else:
            x = cdssm.state_container(self.niter, len(data), self.num, cdssm.s_ts)
            chain_kwargs = {'x': x}
        return chain_kwargs

    def store_x_n(self, x, n):
        if self.fk.cdssm.isobservedat0:
            init_x = x[0]
            x = x[1:]
            self.chain.x[n] = x
            self.chain.init_x[n] = init_x
        else:
            self.chain.x[n] = x
    ...
    def prev_x_copy(self, n):
        if self.fk.cdssm.isobservedat0:
            x = self.chain.x[n-1]
            init_x = self.chain.init_x[n-1]
            return [init_x] + [x_t for x_t in x]
        else:
            return self.chain.x[n-1].copy()
...
class PIMH(X_MCMC):
    ...
    def step0(self):
        ...
        pf.run()
        x = pf.hist.extract_one_trajectory()
        self.store_x_n(x, 0)
...
class CDSSM_PIMH(PIMH, CDSSM_MCMC):
    ...
    def __init__(
        ...
        if smc_options is not None:
            self.smc_options.update(smc_options)
        chain_container_kwargs = self.build_chain_container_kwargs()
        chain_container_kwargs['lpost'] = np.empty(shape=niter)
        self.chain = ssp.ThetaParticles(**chain_container_kwargs)
...
class ICSMC(X_MCMC):
    ...
    def alg_instance(self, n):
        if n == 0:
            ...
            # Consistent with `particles` package: no SMC options for CSMC
            pf = mcmc.CSMC(fk=self.fk, N=self.Nx, xstar=self.x)
        return pf
...
# Run MCMC algorithms (that target latent states only) in parallel:
def mcmc_worker(fk=None, method='pimh', niter=100, Nx=100, num=10):
    """
    Utility for running MCMC algorithms in parallel: to be combined with `multiplexer`
    to run algorithms in parallel
    """
    is_continuous_discrete = hasattr(fk, 'cdssm')
    alg_kwargs = {'fk': fk, 'niter': niter, 'Nx': Nx}
    fk_str = fk.__class__.__name__ if not is_continuous_discrete else fk.sname
    if is_continuous_discrete:
        alg_kwargs.update({'num': num})
    if 'icsmc' in method:
        if method.endswith('_bs'):
            alg_kwargs.update({'backward_step': True})
        else: 
            alg_kwargs.update({'backward_step': False})
    if method == 'pimh':
        alg_class = CDSSM_PIMH if is_continuous_discrete else PIMH
    elif method == 'icsmc':
        alg_class = CDSSM_ICSMC if is_continuous_discrete else ICSMC
    elif method == 'icsmc_bs':
        alg_class = CDSSM_ICSMC if is_continuous_discrete else ICSMC
    else:
        raise ValueError(f"Unknown method: {method}")
    alg = alg_class(**alg_kwargs)
    alg.run()    
    print(f"Method {method} with fk_model {fk_str} took {round(alg.cpu_time, 2)}s for niter={niter}, Nx={Nx}, T={fk.T}")
    idata = alg.to_idata()
    return idata
